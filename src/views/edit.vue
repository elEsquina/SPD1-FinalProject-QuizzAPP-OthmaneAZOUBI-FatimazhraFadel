  ...
  <div class="create-container max-w-2xl mx-auto mt-20 p-6 border border-gray-200 rounded-lg shadow-lg bg-white hover:shadow-xl transition-shadow duration-300">
    ...
    <form @submit.prevent="submitQuiz">
      ...
      <div class="button-container text-center">
        <button type="submit" class="mt-4 p-4 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition duration-300">Edit Quiz</button>
      </div>
    </form>
  </div>
</template>
...
import { addNewQuiz, UpdateQuiz } from '@/firebase/Firestore/addQuiz';
...
export default {
  ...
  async mounted() {
    ...
    if (quizId) {
      ...
      this.quizCategory = data.category;
      this.quizDescription = data.description;
      this.quizTitle = data.title;
      this.quizDifficulty = data.difficulty;
      this.quizDifficulty = this.quizDifficulty.charAt(0).toUpperCase() + this.quizDifficulty.slice(1);
      this.quizImage = data.imageURL;
    }
  }
};
</script>
